...
nonNAIndeciesAndDoablePredictors <- append(nonNAIndecies, which(colnames(training) == "classe"))
nonNAIndeciesAndDoablePredictors <- append(nonNAIndeciesAndDoablePredictors, which(colnames(training) == "user_name"))
excludeFactors <- function(i) { 
  if (class(training[,i]) %in% c("double", "numeric", "integer")) 
  { i }
}
ctrl <- trainControl(preProcOptions = list(thresh = .9))
...
m <- caret::train(y = training[,classeIndex], x = training[,nonNAIndeciesAndDoablePredictors], method="LogitBoost",
                  trControl = ctrl,
