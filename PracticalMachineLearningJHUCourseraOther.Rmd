...
excludeFatFactors <- function(i) { 
  if (class(training[,i]) == "factor" && length(levels(training[,i])) < 53) 
  { i } 
  else if (class(training[,i]) != "factor") 
  { i } 
}
nonNAIndeciesAndDoablePredictors <- lapply(nonNAIndeciesAndDoablePredictors, excludeFatFactors)
nonNAIndeciesAndDoablePredictors <- unlist(nonNAIndeciesAndDoablePredictors)
m <- caret::train(y = training$classe, x = training[,nonNAIndeciesAndDoablePredictors], method="AdaBoost.M1",
                  preProcess = c("scale", "BoxCox",  "pca"),
                  na.action = na.omit) 
summary(m)
```
